...
def rpt_ar_commercial_file(tmp_proyectos_comercial, region, cut_date):
    ...
    for bold_row in range(project_group_row_start-1,r_idx + stage_counter + 1):
        ...
        if bold_row==project_group_row_start -1:
            ...
            cell.border = Border(left=bd_thin, top=bd_thin, right=bd_thick, bottom=bd_thin)
    cells_to_merge="A"+str(project_group_row_start)+":A"+str(len(tmp_proyectos_comercial_excel)+stage_counter+4)
    cell=ws.merge_cells(cells_to_merge)
    cells_to_merge="B"+str(stage_group_row_start+stage_counter)+":B"+str(len(tmp_proyectos_comercial_excel)+stage_counter+3)
    #####
    for bold_column in range(2,15):
        if bold_column == 2:
            cell=ws.cell(row=len(tmp_proyectos_comercial_excel)+stage_counter+4, column=2, value="Total")
        elif bold_column == 5:
            cell=ws.cell(row=len(tmp_proyectos_comercial_excel)+stage_counter+4, column=5, value=np.mean(advance_average))
        elif bold_column == 7:
            cell=ws.cell(row=len(tmp_proyectos_comercial_excel)+stage_counter+4, column=7, value=np.mean(buffer_average))
        elif bold_column == 13:
            cell=ws.cell(row=len(tmp_proyectos_comercial_excel)+stage_counter+4, column=13, value=np.mean(last_week_average))
        elif bold_column ==14:
            cell=ws.cell(row=len(tmp_proyectos_comercial_excel)+stage_counter+4, column=14, value=np.mean(last_month_average))
        else:
            cell=ws.cell(row=len(tmp_proyectos_comercial_excel)+stage_counter+4, column=bold_column)
        if bold_column < 14:
    ...
    with NamedTemporaryFile() as tmp:
        ...
        object_name = "corte_" +  cut_date.strftime('%d-%m-%Y') + "_comercial_" + region + ".xlsx"
